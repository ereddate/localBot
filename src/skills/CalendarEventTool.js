...
exports.CalendarEventTool = void 0;
const Logger_1 = require("../utils/Logger");
class CalendarEventTool {
    constructor() {
        this.name = 'calendar_event_manager';
        this.description = '日历和事件管理工具，用于创建、查询和管理日程事件';
        ...
        try {
            ...
            switch (operation.toLowerCase()) {
                case 'create':
                    ...
                    return this.createEvent(eventTitle, startTime, endTime, params.description);
                case 'get':
                    if (!eventId) {
                ...
                case 'update':
                    ...
                    return this.updateEvent(eventId, eventTitle, startTime, endTime, params.description);
                case 'delete':
                    if (!eventId) {
            ...
            Logger_1.Logger.error('Calendar event tool error', { error: error.message });
            return { success: false, error: `Failed to execute calendar event operation: ${error.message}` };
        }
    }
    ...
    async updateEvent(eventId, title, startTime, endTime, description) {
        // 获取当前事件
        const currentEventResult = await this.getEvent(eventId);
        if (!currentEventResult.success) {
            return currentEventResult;
        }
        const currentEvent = currentEventResult.data.event;
        // 更新事件属性
        const updatedEvent = {
            ...currentEvent,
...
exports.CalendarEventTool = CalendarEventTool;
