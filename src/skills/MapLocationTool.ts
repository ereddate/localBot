...
export class MapLocationTool implements Tool {
  name = 'map_location_service';
  description = '地图和位置服务，提供地理位置查询、距离计算等功能';
  ...
  async execute(params: Record<string, unknown>): Promise<ToolResult> {
    try {
      ...
      Logger.error('Map location tool error', { error: (error as Error).message });
      return { success: false, error: `Failed to execute map location service: ${(error as Error).message}` };
    }
  }

  private async geocode(location: string): Promise<ToolResult> {
  ...
  private async reverseGeocode(lat: number, lng: number): Promise<ToolResult> {
    // 模拟逆地理编码
    const locations = [
    ...
    const foundLocation = locations.find(loc => 
      lat >= loc.lat_range[0] && lat <= loc.lat_range[1] &&
  ...
  private async calculateDistance(origin: string, destination: string): Promise<ToolResult> {
    // 模拟距离计算
    ...
    const o = (originLoc.data as any).coordinates;
    const d = (destLoc.data as any).coordinates;
  ...
  private async getDirections(origin: string, destination: string): Promise<ToolResult> {
    ...
    const distanceData = distanceResult.data as any;
