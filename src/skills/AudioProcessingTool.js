...
exports.AudioProcessingTool = void 0;
const Logger_1 = require("../utils/Logger");
class AudioProcessingTool {
    constructor() {
        this.name = 'audio_processing';
        this.description = '音频处理工具，支持基本的音频操作和处理功能';
    ...
    async execute(params) {
        try {
            ...
            const startTime = params.startTime ? parseFloat(params.startTime) : 0;
            const volume = params.volume ? parseFloat(params.volume) : undefined;
            const speed = params.speed ? parseFloat(params.speed) : undefined;
            ...
            switch (operation.toLowerCase()) {
                ...
                case 'merge':
                    const audioUrls = params.audioUrls || [audioUrl];
            ...
            Logger_1.Logger.error('Audio processing tool error', { error: error.message });
            return { success: false, error: `Failed to execute audio processing operation: ${error.message}` };
        }
    }
    async trimAudio(audioUrl, startTime, duration) {
        // 验证参数
        if (startTime < 0) {
    ...
    async adjustVolume(audioUrl, volume) {
    ...
    async changeSpeed(audioUrl, speed) {
    ...
    async convertFormat(audioUrl, format) {
...
exports.AudioProcessingTool = AudioProcessingTool;
