...
exports.TaxSoftwareIntegrationTool = void 0;
const Logger_1 = require("../utils/Logger");
class TaxSoftwareIntegrationTool {
    constructor() {
        this.name = 'tax_software_integration';
        this.description = '税务软件集成工具，用于与主流税务软件交互';
    ...
    async execute(params) {
        try {
            ...
            const formTypes = params.formTypes;
            ...
            Logger_1.Logger.error('Tax software integration tool error', { error: error.message });
            return { success: false, error: `Failed to execute tax software integration: ${error.message}` };
        }
    }
    async prepareReturn(software, formTypes, data) {
    ...
    async validateReturn(data) {
        ...
        const isValid = Object.values(validationResults).every(result => result.status === 'passed');
        return {
            success: true,
            data: {
    ...
    async exportData(software, data) {
        const validSoftware = ['turbotax', 'hrt_block', 'tax_act', 'proseries', 'athena', 'lacerte'];
        if (!validSoftware.includes(software.toLowerCase())) {
            return { success: false, error: `Unsupported tax software: ${software}. Valid options: ${validSoftware.join(', ')}` };
        }
    ...
    async generateForms(formTypes, data) {
        const validForms = [
            '1040', '1099', 'w2', '1120', '1120s', '1065', '990',
            'sch_a', 'sch_c', 'sch_e', 'dep', 'f8829', 'f8960'
        ];
    ...
    getRequiredFieldsForForm(formType) {
    ...
    getFilledFieldsForForm(formType, data) {
        const result = {};
        // 根据表单类型填充字段
    ...
    getExportFormatForSoftware(software) {
    ...
    populateFormData(formType, data) {
        // 根据表单类型填充表单数据
        const formData = {
            taxYear: data.taxYear || new Date().getFullYear(),
            ...data
    ...
    getRequiredSignatures(formType) {
        switch (formType.toUpperCase()) {
            case '1040':
...
exports.TaxSoftwareIntegrationTool = TaxSoftwareIntegrationTool;
