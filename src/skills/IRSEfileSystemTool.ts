...
export class IRSEfileSystemTool implements Tool {
  name = 'irs_efile_system';
  description = 'IRS电子申报系统集成工具，用于提交税务申报表';
  category = 'system' as const;

  async execute(params: Record<string, unknown>): Promise<ToolResult> {
    try {
      const operation = params.operation as string;
      const returnType = params.returnType as string;
      const payment = parseFloat(params.payment as string) || 0;
      const document = params.document as Record<string, any>;
      ...
      switch (operation.toLowerCase()) {
        ...
        case 'download_acknowledgment':
          const ackId = params.ackId as string;
          if (!ackId) {
        ...
        case 'schedule_payment':
          ...
          return this.schedulePayment(payment, params.dueDate as string);
      ...
      Logger.error('IRS e-file system tool error', { error: (error as Error).message });
      return { success: false, error: `Failed to execute IRS e-file operation: ${(error as Error).message}` };
    }
  }

  private async submitReturn(returnType: string, document: Record<string, any>, payment: number): Promise<ToolResult> {
    // 验证申报表类型
  ...
  private async checkReturnStatus(trackingId: string): Promise<ToolResult> {
    // 验证跟踪ID格式
  ...
  private async downloadAcknowledgment(ackId: string): Promise<ToolResult> {
    // 验证确认ID格式
  ...
  private async schedulePayment(payment: number, dueDate?: string): Promise<ToolResult> {
    // 验证支付金额
    if (payment <= 0) {
