    ...
    try {
      ...
      switch (operation.toLowerCase()) {
        ...
        case 'calculate':
          ...
          const daysToAdd = params.days as number || 0;
          const hoursToAdd = params.hours as number || 0;
          const minutesToAdd = params.minutes as number || 0;
      ...
      Logger.error(`DateTime operation failed`, { error: (error as Error).message });
...
export class TimezoneTool implements Tool {
  ...
  async execute(params: Record<string, unknown>): Promise<ToolResult> {
    try {
      ...
      const sourceTz = params.sourceTz as string;
      const targetTz = params.targetTz as string;
      ...
      Logger.error(`Timezone conversion failed`, { error: (error as Error).message });
      return { success: false, error: (error as Error).message };
    }
  }
}
...
export class CalendarTool implements Tool {
  ...
  description = 'Perform calendar-related operations';
  category = 'system' as const;
  ...
  async execute(params: Record<string, unknown>): Promise<ToolResult> {
    try {
      ...
      Logger.error(`Calendar operation failed`, { error: (error as Error).message });
      return { success: false, error: (error as Error).message };
    }
  }
}