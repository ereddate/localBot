...
exports.ReminderTodoTool = void 0;
const Logger_1 = require("../utils/Logger");
class ReminderTodoTool {
    constructor() {
        this.name = 'reminder_todo_manager';
        this.description = '提醒和待办事项管理工具，用于创建、查询和管理提醒及待办事项';
    ...
    async execute(params) {
        try {
            ...
            Logger_1.Logger.error('Reminder and todo tool error', { error: error.message });
            return { success: false, error: `Failed to execute reminder/todo operation: ${error.message}` };
        }
    }
    async createTask(title, description, dueDate, priority, category) {
    ...
    async createReminder(title, description, triggerDate, priority, category) {
    ...
    async updateTask(taskId, title, description, dueDate, priority, category, completed) {
        const currentTaskResult = await this.getTask(taskId);
        if (!currentTaskResult.success) {
            return currentTaskResult;
        }
        const currentTask = currentTaskResult.data.task;
        // 验证优先级
        const validPriorities = ['low', 'medium', 'high', 'urgent'];
    ...
    async updateReminder(reminderId, title, description, triggerDate, priority, category) {
        ...
        const currentReminder = currentReminderResult.data.reminder;
        // 验证优先级
        const validPriorities = ['low', 'medium', 'high', 'urgent'];
    ...
    async listTasks(category, priority, completed) {
        // 生成模拟任务列表
        const tasks = [];
        // 创建一些示例任务
        for (let i = 0; i < 5; i++) {
    ...
    async listReminders(category, priority) {
        // 生成模拟提醒列表
        const reminders = [];
        // 创建一些示例提醒
        for (let i = 0; i < 4; i++) {
...
exports.ReminderTodoTool = ReminderTodoTool;
