...
export class PasswordGeneratorTool implements Tool {
  ...
  async execute(params: Record<string, unknown>): Promise<ToolResult> {
    try {
      const length = params.length ? parseInt(params.length as string) : 12;
      ...
      const count = params.count ? parseInt(params.count as string) : 1;
      ...
      Logger.error('Password generator tool error', { error: (error as Error).message });
      return { success: false, error: `Failed to generate password: ${(error as Error).message}` };
    }
  }
