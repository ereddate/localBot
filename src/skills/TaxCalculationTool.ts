...
export class TaxCalculationTool implements Tool {
  name = 'tax_calculator';
  description = '税务计算器，支持多种税种的计算和申报';
  ...
  async execute(params: Record<string, unknown>): Promise<ToolResult> {
    try {
      ...
      const deductions = parseFloat(params.deductions as string) || 0;
      const taxYear = parseInt(params.taxYear as string) || new Date().getFullYear();
      const jurisdiction = params.jurisdiction as string || 'federal';
      const filingStatus = params.filingStatus as string || 'single';
      ...
      switch (operation.toLowerCase()) {
        ...
        case 'tax_projection':
          const projectionYears = parseInt(params.projectionYears as string) || 5;
      ...
      Logger.error('Tax calculation tool error', { error: (error as Error).message });
      return { success: false, error: `Failed to execute tax calculation: ${(error as Error).message}` };
    }
  }
  ...
  private async compareTaxBrackets(taxYear: number, jurisdiction: string): Promise<ToolResult> {
    // 返回不同税种的税率对比
  ...
  private async taxProjection(
    ...
    for (let i = 0; i < years; i++) {
      ...
      if (taxResult.success) {
        const taxData = taxResult.data as any;
        projections.push({
          year,
  ...
  private calculateSavingsOpportunities(taxType: string, income: number, deductions: number): any[] {
    const opportunities = [];
