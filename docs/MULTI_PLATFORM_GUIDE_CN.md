...
// 发送消息
await telegram.sendMessage('user_id', '你好，来自LocalBot！');

// 发送文件
await telegram.sendFile('user_id', '/path/to/file.pdf', '查看此文件');
```
...
// 发送消息
await discord.sendMessage('user_id', '你好，来自LocalBot！');

// 发送文件
await discord.sendFile('user_id', '/path/to/file.pdf', '查看此文件');
```
...
// 发送消息
await slack.sendMessage('channel_id', '你好，来自LocalBot！');

// 发送文件
await slack.sendFile('channel_id', '/path/to/file.pdf', '查看此文件');
```
...
// 发送消息
await whatsapp.sendMessage('phone_number', '你好，来自LocalBot！');

// 发送文件
await whatsapp.sendFile('phone_number', '/path/to/file.pdf', '查看此文件');
```
...
platformManager.onError((error, platform) => {
  console.error(`平台 ${platform} 错误:`, error);
});

// 获取平台状态
const status = platformManager.getPlatformStatus();
console.log('平台状态:', status);
```
...
platformManager.onMessage(async (message) => {
  if (message.platform === 'telegram') {
    ...
    console.log('Telegram用户:', metadata.username);
  }
...
platformManager.onMessage(async (message) => {
  if (message.platform === 'discord') {
    // 处理Discord特定功能
    const metadata = message.metadata as any;
    console.log('Discord用户:', metadata.username);
    console.log('Discord服务器:', metadata.guildId);
  }
...
1. **错误处理**
   ...
   ```typescript
   platformManager.onError((error, platform) => {
     Logger.error(`平台 ${platform} 错误`, { error: error.message });
