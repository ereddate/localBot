...
本文档介绍如何将LocalBot部署到Android、鸿蒙(HarmonyOS)和iOS平台。
...
iOS平台的部署方案请参考[iOS部署指南](IOS_DEPLOYMENT.md)。
...
1. **选择方案**
   ...
   根据您的需求选择合适的部署方案：
   - 新项目：推荐Ionic（快速开发）
   - 现有Web应用：推荐Capacitor（最小改动）
   - 追求性能：推荐React Native（原生体验）
   - 快速部署：推荐PWA（无需审核）
...
3. **部署应用**
   ...
   按照[iOS部署指南](IOS_DEPLOYMENT.md)中的详细步骤进行部署。
...
- iOS用户：使用Ionic方案，快速开发，跨平台
- 开发者：考虑使用仓颉语言进行原生开发

详细部署指南：
- [Android部署](#android平台部署)
- [鸿蒙部署](#鸿蒙平台部署)
- [iOS部署](IOS_DEPLOYMENT.md)
